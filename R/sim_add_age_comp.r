...
sim_add_age_comp <- function(dat_struct=NULL,comp_year=-1,comp_seas=-1,comp_fleet=-1,comp_Nsamp=-1,comp_matrix=NULL)
{
    new_dat_struct <- NULL
    ...
    if (!is.null(dat_struct) && comp_year > 0 && comp_seas > 0 && comp_fleet > 0 && comp_Nsamp > 0 && !is.null(comp_matrix))
    {
        if (comp_fleet <= (dat_struct$Nfleet+dat_struct$Nsurveys) && comp_seas <= dat_struct$nseas)
        {
            ...
            {
                new_dat_struct$agecomp <- rbind(dat_struct$agecomp,c(comp_year,comp_seas,comp_fleet,3,0,1,-1,-1,comp_Nsamp,as.vector(comp_matrix[1,]),as.vector(comp_matrix[2,])))
            } else {
